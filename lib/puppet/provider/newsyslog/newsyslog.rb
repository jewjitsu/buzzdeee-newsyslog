...
Puppet::Type.type(:newsyslog).provide(
  ...
  record_line :parsed,
    ...
    :pre_gen => proc { |hash|
      if (hash.has_key? :owner \
          and hash[:owner] != nil \
          and hash[:owner] != :absent) \
          or (hash.has_key? :group \
          and hash[:group] != nil \
          and hash[:group] != :absent)
      ...
      if (hash[:remainder] == :absent or hash[:remainder] == nil)
        hash[:remainder] = ''
      end
      ...
      if (hash.has_key? :monitor)
        if (hash.has_key? :flags and hash[:flags].match(/M/))
          ...
          raise Puppet::ParseError, "Monitor mail receiver must be " +
      ...
      if (hash.has_key? :command \
          and (hash.has_key? :pidfile \
          or hash.has_key? :sigtype))
        raise Puppet::ParseError, "Given command parameter and pidfile or " +
                                  "sigtype. Parameter command is mutually " +
                                  "exclusive to the others."
      end

      if ((hash.has_key? :pidfile \
           and not hash.has_key? :sigtype) \
           or (hash.has_key? :sigtype \
           and not hash.has_key? :pidfile))
        raise Puppet::ParseError, "Parameters pidfile and sigtype must be " +
                                  "specified together."
      end
